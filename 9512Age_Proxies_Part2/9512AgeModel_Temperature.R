...
setwd("C:/Users/Sofia Barragan Monti/Documents/PhDBFMultivariate/TEAtlanticBF/9512Age_Proxies_Part2") 
...
## Melonis with euqation of Kristjandottir et al., 2007 from (pending, not submitted yet)
BWTMel_Kr<-read.csv(file="GeoB9512-5_BWTMelonis(Krist).csv", 
                 header=TRUE, stringsAsFactors=FALSE,sep=",",na.strings="-", check.names = F)
colnames(BWTMel_Kr)<- c("Depth [cm]", "BWT[deg-C]")

## Melonis with our equation from (pending, not submitted yet)
BWTMel_Eq4<-read.csv(file="GeoB9512-5_BWTMelonis(Barr).csv", 
                 header=TRUE, stringsAsFactors=FALSE,sep=",",na.strings="-", check.names = F)
colnames(BWTMel_Eq4)<- c("Depth [cm]", "BWT[deg-C]")

## Melonis with our equation O2 corrected from (pending, not submitted yet)
BWTMel_O2<-read.csv(file="GeoB9512-5_BWTMelonis(Barr_OxCorr).csv", 
                           header=TRUE, stringsAsFactors=FALSE,sep=",",na.strings="-", check.names = F)
colnames(BWTMel_O2)<- c("Depth [cm]", "BWT[deg-C]")
...
#Bottom Water Temperatures (BWT) Melonis (Kristjandottir Equation)
BWT_MCAG_MelKr<-sapply(BWTMel_Kr[,1], Bacon.Age.d) # Extract ages with Bacon for BWT_Uvi
BWTneff_MelKr<-dim(BWT_MCAG_MelKr)[1] # Determine number of columns
BWTchron.dat_MelKr<-t(BWT_MCAG_MelKr)[,(BWTneff_MelKr-nmc+1):BWTneff_MelKr] # Tailor to desired number of iterations (nmc)
BWTchron.median_MelKr<-apply(BWTchron.dat_MelKr,1,median) # Calculate median age

#Bottom Water Temperatures (BWT) Melonis (This Study Equation)
BWT_MCAG_Mel_Eq4<-sapply(BWTMel_Eq4[,1], Bacon.Age.d) # Extract ages with Bacon for BWT_Uvi
BWTneff_Mel_Eq4<-dim(BWT_MCAG_Mel_Eq4)[1] # Determine number of columns
BWTchron.dat_Mel_Eq4<-t(BWT_MCAG_Mel_Eq4)[,(BWTneff_Mel_Eq4-nmc+1):BWTneff_Mel_Eq4] # Tailor to desired number of iterations (nmc)
BWTchron.median_Mel_Eq4<-apply(BWTchron.dat_Mel_Eq4,1,median) # Calculate median age

#Bottom Water Temperatures (BWT) Melonis (This Study Equation Oxygen Corrected)
BWT_MCAG_Mel_O2<-sapply(BWTMel_O2[,1], Bacon.Age.d) # Extract ages with Bacon for BWT_Uvi
BWTneff_Mel_O2<-dim(BWT_MCAG_Mel_O2)[1] # Determine number of columns
BWTchron.dat_Mel_O2<-t(BWT_MCAG_Mel_O2)[,(BWTneff_Mel_O2-nmc+1):BWTneff_Mel_O2] # Tailor to desired number of iterations (nmc)
BWTchron.median_Mel_O2<-apply(BWTchron.dat_Mel_O2,1,median) # Calculate median age
...
for(j in 1:length(BWTUvi[,2])){BWTsig.prx[j,]<-rnorm(n=nmc,sd=0.7)} # Put noise into matrix

BWTdat.unc<-BWTdat.prx+BWTsig.prx # Add noise to data and store in matrix

#BWT from Melonis Mg/Ca (Kristjandottir)
BWTdat.unc_Mel_Kr<-matrix(0,nrow=length(BWTMel_Kr[,2]),ncol=nmc,byrow=FALSE) # Prepare matrix for BWT_Uvi + noise
BWTdat.prx_Mel_Kr<-matrix(BWTMel_Kr[,2],nrow=length(BWTMel_Kr[,2]),ncol=nmc,byrow=FALSE) # Prepare matrix with data
BWTsig.prx_Mel_Kr<-matrix(0,nrow=length(BWTMel_Kr[,2]),ncol=nmc,byrow=FALSE) # Prepare matrix for noise
for(j in 1:length(BWTMel_Kr[,2])){BWTsig.prx_Mel_Kr[j,]<-rnorm(n=nmc,sd=1.40)} # Put noise into matrix

BWTdat.unc_Mel_Kr<-BWTdat.prx_Mel_Kr+BWTsig.prx_Mel_Kr # Add noise to data and store in matrix

#BWT from Melonis Mg/Ca (This Study - Equation 4)
BWTdat.unc_Mel_Eq4<-matrix(0,nrow=length(BWTMel_Eq4[,2]),ncol=nmc,byrow=FALSE) # Prepare matrix for BWT_Uvi + noise
BWTdat.prx_Mel_Eq4<-matrix(BWTMel_Eq4[,2],nrow=length(BWTMel_Eq4[,2]),ncol=nmc,byrow=FALSE) # Prepare matrix with data
BWTsig.prx_Mel_Eq4<-matrix(0,nrow=length(BWTMel_Eq4[,2]),ncol=nmc,byrow=FALSE) # Prepare matrix for noise
for(j in 1:length(BWTMel_Eq4[,2])){BWTsig.prx_Mel_Eq4[j,]<-rnorm(n=nmc,sd=0.76)} # Put noise into matrix

BWTdat.unc_Mel_Eq4<-BWTdat.prx_Mel_Eq4+BWTsig.prx_Mel_Eq4 # Add noise to data and store in matrix

#BWT from Melonis Mg/Ca (This Study - O2 correcter)
BWTdat.unc_Mel_O2<-matrix(0,nrow=length(BWTMel_O2[,2]),ncol=nmc,byrow=FALSE) # Prepare matrix for BWT_Uvi + noise
BWTdat.prx_Mel_O2<-matrix(BWTMel_O2[,2],nrow=length(BWTMel_O2[,2]),ncol=nmc,byrow=FALSE) # Prepare matrix with data
BWTsig.prx_Mel_O2<-matrix(0,nrow=length(BWTMel_O2[,2]),ncol=nmc,byrow=FALSE) # Prepare matrix for noise
for(j in 1:length(BWTMel_O2[,2])){BWTsig.prx_Mel_O2[j,]<-rnorm(n=nmc,sd=0.58)} # Put noise into matrix

BWTdat.unc_Mel_O2<-BWTdat.prx_Mel_O2+BWTsig.prx_Mel_O2 # Add noise to data and store in matrix
...
#d18O of Cibicidoides (For Melonis)
d18OMedAge_Mel<-matrix(NA, nrow=length(BWTchron.median_Mel_Eq4), ncol=nmc) # Matrix for interpolated values
for (i in 1:nmc)
{
  d18OMedAge_Mel[,i]<-approx(d18Ochron.dat[,i],d18Odat.unc[,i],BWTchron.median_Mel_Eq4)$y 
}
d18OMedAge.mean_Mel<-matrix(apply(d18OMedAge_Mel,1,mean,na.rm=TRUE)) # Calculate mean d18O

#d18O of Cibicidoides (For Melonis)
d18OMedAge_Mel2<-matrix(NA, nrow=length(BWTchron.median_Mel_O2), ncol=nmc) # Matrix for interpolated values
for (i in 1:nmc)
{
  d18OMedAge_Mel2[,i]<-approx(d18Ochron.dat[,i],d18Odat.unc[,i],BWTchron.median_Mel_O2)$y 
}
d18OMedAge.mean_Mel2<-matrix(apply(d18OMedAge_Mel2,1,mean,na.rm=TRUE)) # Calculate mean d18O
...
#BWT data - Melonis (Kristjandottir)
BWTMedAge_Mel_Kr<-matrix(NA, nrow=length(BWTchron.median_MelKr), ncol=nmc) # Matrix for interpolated values BWT_Uvi
for (i in 1:nmc)
{
  BWTMedAge_Mel_Kr[,i]<-approx(BWTchron.dat_MelKr[,i],BWTdat.unc_Mel_Kr[,i],BWTchron.median_MelKr)$y 
}
BWTMedAge.mean_Mel_Kr<-matrix(apply(BWTMedAge_Mel_Kr,1,mean,na.rm=TRUE)) # Calculate mean d18O

#BWT data - Melonis (Eq 4)
BWTMedAge_Mel_Eq4<-matrix(NA, nrow=length(BWTchron.median_Mel_Eq4), ncol=nmc) # Matrix for interpolated values BWT_Uvi
for (i in 1:nmc)
{
  BWTMedAge_Mel_Eq4[,i]<-approx(BWTchron.dat_Mel_Eq4[,i],BWTdat.unc_Mel_Eq4[,i],BWTchron.median_Mel_Eq4)$y 
}
BWTMedAge.mean_Mel_Eq4<-matrix(apply(BWTMedAge_Mel_Eq4,1,mean,na.rm=TRUE)) # Calculate mean d18O

#BWT data - Melonis (This Study_Oxygen Corrected)
BWTMedAge_Mel_O2<-matrix(NA, nrow=length(BWTchron.median_Mel_O2), ncol=nmc) # Matrix for interpolated values BWT_Uvi
for (i in 1:nmc)
{
  BWTMedAge_Mel_O2[,i]<-approx(BWTchron.dat_Mel_O2[,i],BWTdat.unc_Mel_O2[,i],BWTchron.median_Mel_O2)$y 
}
BWTMedAge.mean_Mel_O2<-matrix(apply(BWTMedAge_Mel_O2,1,mean,na.rm=TRUE)) # Calculate mean d18O
...
d18Ow_Mel <- ((BWTMedAge_Mel_Kr-16.9+(4*d18OMedAge_Mel))/4)             #d18Osw using Schackelton, 1984 
d18Ow.mean_Mel <-matrix(apply(d18Ow_Mel,1,mean,na.rm=TRUE))

d18Ow_Mel2 <- ((BWTMedAge_Mel_Eq4-16.9+(4*d18OMedAge_Mel))/4)             #d18Osw using Schackelton, 1984 
d18Ow.mean_Mel2 <-matrix(apply(d18Ow_Mel2,1,mean,na.rm=TRUE))

d18Ow_MelO2 <- ((BWTMedAge_Mel_O2-16.9+(4*d18OMedAge_Mel2))/4)             #d18Osw using Schackelton, 1984
d18Ow.mean_MelO2 <-matrix(apply(d18Ow_MelO2,1,mean,na.rm=TRUE))
...
QtsBWT_Mel_Eq4 <- t(apply(BWTMedAge_Mel_Eq4,1,quantile,probs=probs,na.rm=TRUE))
QtsBWT_Mel_Kr <- t(apply(BWTMedAge_Mel_Kr,1,quantile,probs=probs,na.rm=TRUE))
QtsBWT_Mel_O2 <- t(apply(BWTMedAge_Mel_O2,1,quantile,probs=probs,na.rm=TRUE))
Qts18Ow <- t(apply(d18Ow,1,quantile,probs=probs,na.rm=TRUE))
Qts18Ow_Mel <- t(apply(d18Ow_Mel,1,quantile,probs=probs,na.rm=TRUE))
Qts18Ow_Mel2 <- t(apply(d18Ow_Mel2,1,quantile,probs=probs,na.rm=TRUE))
Qts18Ow_Mel_O2 <- t(apply(d18Ow_MelO2,1,quantile,probs=probs,na.rm=TRUE))
...
dfBWT_Mel_Kr<-data.frame(cbind(BWTMel_Kr[,1],BWTchron.median_MelKr/1000,BWTMedAge.mean_Mel_Kr,QtsBWT_Mel_Kr)) # Merge fields
colnames(dfBWT_Mel_Kr)<-c("Depth [cm]","Median Age [kyrs]","BWT [deg C]","Q0.025","Q0.16", "Q0.84", "Q0.975")
write.csv(dfBWT_Mel_Kr,file=sprintf("OPGeoB9512-5_BWTMelonis_Kristjandottir.csv"), row.names=FALSE)

dfBWT_Mel_Eq4<-data.frame(cbind(BWTMel_Eq4[,1],BWTchron.median_Mel_Eq4/1000,BWTMedAge.mean_Mel_Eq4,QtsBWT_Mel_Eq4)) # Merge fields
colnames(dfBWT_Mel_Eq4)<-c("Depth [cm]","Median Age [kyrs]","BWT [deg C]","Q0.025","Q0.16", "Q0.84", "Q0.975")
write.csv(dfBWT_Mel_Eq4,file=sprintf("OPGeoB9512-5_BWTMelonis_ThisStudyEq4.csv"), row.names=FALSE)

dfBWT_Mel_O2<-data.frame(cbind(BWTMel_O2[,1],BWTchron.median_Mel_O2/1000,BWTMedAge.mean_Mel_O2,QtsBWT_Mel_O2)) # Merge fields
colnames(dfBWT_Mel_O2)<-c("Depth [cm]","Median Age [kyrs]","BWT [deg C]","Q0.025","Q0.16", "Q0.84", "Q0.975")
write.csv(dfBWT_Mel_O2,file=sprintf("OPGeoB9512-5_BWTMelonis_ThisStudyOxygCorr.csv"), row.names=FALSE)
...
dfdwMel<-data.frame(cbind(BWTchron.median_MelKr/1000,d18Ow.mean_Mel,Qts18Ow_Mel)) # Merge fields
colnames(dfdwMel)<-c("Median Age [kyrs]","dw [permil]","Q0.025","Q0.16", "Q0.84", "Q0.975")
write.csv(dfdwMel,file=sprintf("OPGeoB9512-5_dw_MelKr.csv"), row.names=FALSE)

dfdwMel2<-data.frame(cbind(BWTchron.median_Mel_Eq4/1000,d18Ow.mean_Mel2,Qts18Ow_Mel2)) # Merge fields
colnames(dfdwMel2)<-c("Median Age [kyrs]","dw [permil]","Q0.025","Q0.16", "Q0.84", "Q0.975")
write.csv(dfdwMel2,file=sprintf("OPGeoB9512-5_dw_MelOurStudy.csv"), row.names=FALSE)

dfdwMelO2<-data.frame(cbind(BWTchron.median_Mel_O2/1000,d18Ow.mean_MelO2,Qts18Ow_Mel_O2)) # Merge fields
colnames(dfdwMelO2)<-c("Median Age [kyrs]","dw [permil]","Q0.025","Q0.16", "Q0.84", "Q0.975")
write.csv(dfdwMelO2,file=sprintf("OPGeoB9512-5_dw_MelOurStudy(OxygCorrected).csv"), row.names=FALSE)
...
dfBWT_Mel_Kr<-read.csv("OPGeoB9512-5_BWTMelonis_Kristjandottir.csv", check.names=F)
dfBWT_Mel_Eq4<-read.csv("OPGeoB9512-5_BWTMelonis_ThisStudyEq4.csv", check.names=F)
dfBWT_Mel_O2<-read.csv("OPGeoB9512-5_BWTMelonis_ThisStudyOxygCorr.csv", check.names=F)

dfdw<-read.csv("OPGeoB9512-5_dw.csv", check.names=FALSE)
dfdw_Mel_Kr<-read.csv("OPGeoB9512-5_dw_MelKr.csv", check.names=FALSE)
dfdw_Mel_eq4<-read.csv("OPGeoB9512-5_dw_MelOurStudy.csv", check.names=FALSE)
dfdw_Mel_O2<-read.csv("OPGeoB9512-5_dw_MelOurStudy(OxygCorrected).csv", check.names=FALSE)
...
PhThPlot <- ggplot()+
  ...
  geom_errorbar(PhTh_A,mapping=aes(y= `Age [kyrs]`,xmin = `Pa/Th232` - Error232,
                ...
                colour= "#66CC99", linetype = 1, width = 0.1)+
  ...
  geom_errorbar(PhTh_A,mapping=aes(y= `Age [kyrs]`,xmin = `Pa/Th238` - Error238,
                ...
                colour= "darkgray", linetype = 1, width = 0.1)+
...
#Cibicidoides %
ea1 <- read.csv("9512_EA.csv", check.names=FALSE)
...
#Current Velocity Plot
CurrentVelocityPlot <- ggplot(ea1, aes(`Current Velocities (2)`, `Age [ka]`))+
  geom_lineh()+
  geom_vline(xintercept = 0.13, linetype = 2, linewidth = 0.5, alpha=0.5)+
  scale_y_reverse(guide="prism_minor",
                  breaks = seq(50,0,by=-5),
                  limits = c(27,0),
                  expand = c(0,0),
  ...
  labs(title="Current Velocity (m/s)")+ xlab("")+ ylab("Age (Kyrs)")+
  theme_classic()+
  theme(axis.line.y =element_blank(), 
        axis.title.y=element_blank(),
        axis.text.y=element_blank(),
...
#Figure 4
ggarrange(PhThPlot, CibSpPlot, d13Cplot2, CurrentVelocityPlot, BWOxPlot, dPlot, nrow=1)
...
d18Oplotsw <- ggplot(dfdw, aes(`dw [permil]`, `Median Age [kyrs]`))+
  ...
  geom_ribbon(dfdw, mapping=aes(xmin = `Q0.16`, xmax = `Q0.84`, y=`Median Age [kyrs]`),
              inherit.aes = F, fill = "black", alpha=0.3)+ 
  ...
  geom_point(dfdw, mapping=aes(`dw [permil]`, `Median Age [kyrs]`), 
             colour="black", size=1, alpha=0.5)+
  geom_ribbon(dfdw_Mel_Kr, mapping=aes(xmin = `Q0.16`, xmax = `Q0.84`, y=`Median Age [kyrs]`),
              inherit.aes = F, fill = "#7d9ec2", alpha=0.3)+
  geom_lineh(dfdw_Mel_Kr, mapping=aes(`dw [permil]`, `Median Age [kyrs]`),
             colour="#7d9ec2", linetype = 2, linewidth = 0.25)+
  geom_point(dfdw_Mel_Kr, mapping=aes(`dw [permil]`, `Median Age [kyrs]`),
             colour="blue", size = 1, alpha = 0.5)+
  geom_ribbon(dfdw_Mel_O2, mapping=aes(xmin = `Q0.16`, xmax = `Q0.84`, y=`Median Age [kyrs]`),
              inherit.aes = F, fill = "#A6B578", alpha=0.3)+
  geom_lineh(dfdw_Mel_O2, mapping=aes(`dw [permil]`, `Median Age [kyrs]`),
             colour="#A6B578", linetype = 2, linewidth = 0.25)+
  geom_point(dfdw_Mel_O2, mapping=aes(`dw [permil]`, `Median Age [kyrs]`),
             colour="darkgreen", size = 1, alpha = 0.5)+
  geom_ribbon(dfdw_Mel_eq4, mapping=aes(xmin = `Q0.16`, xmax = `Q0.84`, y=`Median Age [kyrs]`),
              inherit.aes = F, fill = "#993399", alpha=0.3)+
  geom_lineh(dfdw_Mel_eq4, mapping=aes(`dw [permil]`, `Median Age [kyrs]`),
             colour="#993399", linetype = 2, linewidth = 0.25)+
  geom_point(dfdw_Mel_eq4, mapping=aes(`dw [permil]`, `Median Age [kyrs]`),
             colour="#993399", size = 1, alpha = 0.5)+
  # geom_ribbon(dfdw4, mapping=aes(xmin = `Q0.16`, xmax = `Q0.84`, y=`Median Age [kyrs]`),
  #             inherit.aes = F, fill = "#A6B578", alpha=0.3)+
  # geom_lineh(dfdw4, mapping=aes(`dw3 [permil]`, `Median Age [kyrs]`),
  #            colour="#A6B578", linetype = 2, linewidth = 0.25)+
  # geom_point(dfdw4, mapping=aes(`dw3 [permil]`, `Median Age [kyrs]`),
#            colour="darkgreen", size = 1, alpha = 0.5)+
...
BWTplot<- ggplot(dfBWT, aes(`BWT [deg C]`, `Median Age [kyrs]`))+
  ...
  geom_ribbon(dfBWT_Mel_Kr, mapping=aes(xmin = `Q0.16`, xmax = `Q0.84`), 
              fill = "#629CFF", alpha=0.2)+
  geom_lineh(dfBWT_Mel_Kr, mapping=aes(`BWT [deg C]`, `Median Age [kyrs]`), 
             color="blue", linetype = 2, size = 0.25)+
  geom_point(dfBWT_Mel_Kr, mapping=aes(`BWT [deg C]`, `Median Age [kyrs]`), 
             color="blue", size = 1, alpha = 0.5)+
  geom_ribbon(dfBWT_Mel_Eq4, mapping=aes(xmin = `Q0.16`, xmax = `Q0.84`), 
              fill = "#993399", alpha=0.2)+
  geom_lineh(dfBWT_Mel_Eq4, mapping=aes(`BWT [deg C]`, `Median Age [kyrs]`), 
             color="#993399", linetype = 2, size = 0.25)+
  geom_point(dfBWT_Mel_Eq4, mapping=aes(`BWT [deg C]`, `Median Age [kyrs]`), 
             color="#993399", size = 1, alpha = 0.5)+
  geom_ribbon(dfBWT_Mel_O2, mapping=aes(xmin = `Q0.16`, xmax = `Q0.84`), 
              fill = "#A6B578", alpha=0.2)+
  geom_lineh(dfBWT_Mel_O2, mapping=aes(`BWT [deg C]`, `Median Age [kyrs]`), 
             color="darkgreen", linetype = 2, size = 0.25)+
  geom_point(dfBWT_Mel_O2, mapping=aes(`BWT [deg C]`, `Median Age [kyrs]`), 
...
##TraceElements
TE_9512Mel <- read.csv(file="GeoB9512-5_ElementsCalciumRatios_Mel.csv", check.names = F, header=TRUE, 
                   stringsAsFactors=FALSE,sep=",",na.strings="-")

TE_9512Uvi <- read.csv(file="GeoB9512-5_ElementsCalciumRatios_Uvi.csv", check.names = F, header=TRUE, 
                    stringsAsFactors=FALSE,sep=",",na.strings="-")
...
MgCa9512 <- ggplot()+
  ...
  geom_rect(mapping = aes(ymin = ymin, ymax = ymax, xmin = xmin, xmax = xmax),
            data = YD, alpha = 0.6, fill = "#c7c7c7", inherit.aes = FALSE)+
  geom_lineh(TE_9512Mel, mapping=aes(`Mg/Ca  [mmol/mol]`, `Median Age [kyrs]`),
             colour="#CC6666", linetype = 1, size = 0.25)+
  # geom_errorbar(PhTh_C,mapping=aes(y= `Age [kyrs]`,xmin = `231Pa/230Th` - `2SD`,
  #                                  xmax = `231Pa/230Th` + `2SD`),
  #               colour="#CC6666", linetype = 1, width = 0.1, size=0.1)+
  geom_lineh(TE_9512Uvi, mapping=aes(`Mg/Ca  [mmol/mol]`, `Age [kyrs BP (2000)]`),
             colour="black", linetype = 1, size = 0.25)+
  scale_y_reverse(guide="prism_minor",
                  breaks = seq(180,0,by=-5),
                  limits = c(27,0),
...
MnCa9512 <- ggplot()+
  ...
  geom_rect(mapping = aes(ymin = ymin, ymax = ymax, xmin = xmin, xmax = xmax),
            data = YD, alpha = 0.6, fill = "#c7c7c7", inherit.aes = FALSE)+
  geom_lineh(TE_9512Mel, mapping=aes(`Mn/Ca   [mmol/mol]`, `Median Age [kyrs]`),
             colour="#CC6666", linetype = 1, size = 0.25)+
  # geom_errorbar(PhTh_C,mapping=aes(y= `Age [kyrs]`,xmin = `231Pa/230Th` - `2SD`,
  #                                  xmax = `231Pa/230Th` + `2SD`),
  #               colour="#CC6666", linetype = 1, width = 0.1, size=0.1)+
  geom_lineh(TE_9512Uvi, mapping=aes(`Mn/Ca   [mmol/mol]`, `Age [kyrs BP (2000)]`),
             colour="black", linetype = 1, size = 0.25)+
  scale_y_reverse(guide="prism_minor",
                  breaks = seq(180,0,by=-5),
                  limits = c(27,0),
...
ggarrange(PhThPlot, d13Cplot2,MgCa9512, MnCa9512,  BWTplot, d18Oplotsw,nrow=1)
...
ggarrange(AbundSp, TestPlot, SSPlot, dPlot, 
